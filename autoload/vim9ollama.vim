...
export def GetModelConfig(model_key: string): dict<any>
...
export def OnSrvError(ch: channel, msg: string)
    echom msg
enddef

export def OnResponse(ch: channel, msg: string)
...
export def GetContext(linesAround: number = 50): string
  const cur = line(".")
...
export def CallOllamaApi(prompt: string, model_key: string, Callback: func)
...
export def BuildOllamaPrompt(model_key: string, instruction: string, selected: string, filetype: string, ctx: string): string
...
export def OllamaAsk(instruction: string)
...
export def OllamaChange(instruction: string)
...
export def OllamaRead(instruction: string)
  var selected = GetSelectedText()
  var ctx = GetContext(get(g:, "ollama_context_lines", 50))
  var final_prompt = BuildOllamaPrompt(
...
export def OnResponseComplete(ch: channel, msg: string)
...
export def OnError(ch: channel, msg: string)
  echoe "ERROR: " .. msg
enddef
